...
async def google_oauth_start():
    ...
    if not settings.GOOGLE_CLIENT_ID:
        raise HTTPException(400, "Google OAuth não configurado. Defina GOOGLE_CLIENT_ID no .env")
...
async def google_callback(code: str, db: AsyncSession = Depends(get_db)):
    """Callback OAuth Google — cria/atualiza usuário e retorna token JWT."""
    try:
        from google_auth_oauthlib.flow import Flow
        ...
        return RedirectResponse(f"/?token={jwt_token}&name={name}")

    except Exception as e:
        raise HTTPException(500, f"Erro no OAuth Google: {str(e)}")
